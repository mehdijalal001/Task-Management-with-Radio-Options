...
import { TasksData, TaskspopupviewComponent } from '../components/tasks/taskspopupview/taskspopupview.component';
...
export class TasksdialogService {
  ...
  //--------------dialog-------//
  openTasksDialog(data,id, action,text,type) {
    const resDialog = new TasksData(data,id,action,text,type);

    const dialogRef = this.dialog.open(TaskspopupviewComponent, {
      maxWidth: '500px',
      data: resDialog
    });
